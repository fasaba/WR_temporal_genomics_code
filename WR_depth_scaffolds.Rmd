...
f = ggplot(NWR1, aes(Sample, AveScaffoldDepth)) + 
  geom_violin(aes(Sample, AveScaffoldDepth), fill="gray50", colour="gold2", alpha=.4, lwd=.3) +
  theme_bw() +
  scale_x_discrete(labels=as.character(NWR1_newID)) +
  scale_y_continuous(limits=c(0, 25)) +
  stat_summary(fun="median", geom="crossbar", width=.09, lwd=.3, colour="gray10") +
  theme_bw() + 
  theme(legend.position = "none", axis.title=element_text(size=6), axis.text.x=element_text(size=4, angle=40, hjust=1, colour='black'), axis.text.y=element_text(size=6, colour='gray10'), panel.grid.major.y = element_line(size=.1, colour='gray70')) +
...
g = ggplot(NWR2, aes(Sample, AveScaffoldDepth)) + 
  geom_violin(aes(Sample, AveScaffoldDepth), fill="gray50", colour="darkorange", alpha=.4, lwd=.3) +
...
h = ggplot(SWR1, aes(Sample, AveScaffoldDepth)) + 
  geom_violin(aes(Sample, AveScaffoldDepth), fill="gray50", colour="royalblue4", alpha=.4, lwd=.3) +
  theme_bw() +
...
i = ggplot(SWR2, aes(Sample, AveScaffoldDepth)) + 
  geom_violin(aes(Sample, AveScaffoldDepth), fill="gray50", colour="steelblue3", alpha=.4, lwd=.3) +
  theme_bw() +
...
```{r}
J = plot_grid(f, g, h, i, nrow=2, ncol=2, labels=c('A', 'B', 'C', 'D'), label_size = 7)
J
```


```{r}
pdf('WR_v3_FigS2_depths_v1.pdf', height=4, width=5, useDingbats = F)
J
dev.off()

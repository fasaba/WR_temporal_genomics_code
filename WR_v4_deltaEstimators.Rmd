...
Remove the 6 samples that either: have depth <5x (4 samples) and/or have error rates >1% (4 extra samples) for comparisons of RoH and Froh
```{r}
...
Group comparison
```{r}
wilcox.test(NWRpreFnoDRC$Froh, NWRpostFnoDRC$Froh, paired=F, alternative = "less")
```
...
#### Group comparison
```{r}
wilcox.test(NWRpreFnoDRC$ave_RoH_len, NWRpostFnoDRC$ave_RoH_len, paired=F, alternative = "less")
```
...
```{r}
# With the values from the table above, do median post-bottleneck value minus median pre-bottleneck value, divided by the median pre-bottleneck value
(291588.0-217302.0)/217302.0

# the result is the increase in average RoH length between NWRpre and post (excluding DRC)
```
...
#### Group comparison
```{r}
wilcox.test(NWRpreFnoDRC$max_RoH_len, NWRpostFnoDRC$max_RoH_len, paired=F, alternative = "less")
```
...
```{r}
# With the values from the table above, do median post-bottleneck value minus median pre-bottleneck value, divided by the median pre-bottleneck value
(5808820-4330095)/4330095

# the result is the increase in maximum RoH length between NWRpre and post (excluding DRC)
```

...
A = ggplot(NWR1, aes(variable, value, colour=variable)) + 
    theme_bw() + 
  theme(legend.position = "none", axis.title.y=element_text(size=7), axis.text.x=element_text(size=7, angle=40, hjust=1, colour='black'), axis.text.y=element_text(size=7, colour='black')) +
...
```{r}
pdf('WR_v3_FigS7_v3.pdf', height=5, width=6, useDingbats = F)
plot_grid(p1, p2, ncol=1, nrow = 2)
dev.off()
```

```{r}
# png('WR_v3_FigS7_v2.png', height=5, width=6,  units = 'in', res = 300)
# plot_grid(p1, p2, ncol=1, nrow = 2)
# dev.off()
```
...
roh_len_ss = filter(roh_len_info_filt_above1Mb_ss, !is.na(time_window)) %>% 
  ...
  # embellish
  scale_y_continuous(limits=c(0, 42e6), breaks=c(1.5e6, 10e6, 20e6, 30e6, 40e6), labels=c(1.5, 10, 20, 30, 40)) +
  theme_bw() + theme(legend.position = "none", axis.title=element_text(size=7), axis.text=element_text(size=7, colour='black')) +
  labs(x=NULL, y="RoH length (Mbp)")
...
```{r}
png('WR_v3_FigS9_v1.png', height=4, width=5, units = 'in', res = 300)
plot_grid(roh_len_ss, froh_ss, nrow=1, ncol=2, labels=c('A','B'), label_size=7)
dev.off()
```

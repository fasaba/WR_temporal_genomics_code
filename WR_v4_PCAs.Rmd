...
A = ggplot(evec, aes(x=PC1, y=PC2)) +
  ...
  geom_point(aes(colour = window_country, shape = window_country), alpha=.7, size=4) + 
  ...
  #scale_shape_manual(values=c(7, 1, 10, 17, 18, 15, 9, 8)) +
...
nwr = ggplot(evecNWR, aes(x=PC1, y=PC2)) +
  ...
  # plot data points (2 legend alternatives)
  geom_point(aes(colour = window_country, shape = window_country), alpha=.7, size=3) + 
  
  # assign colors and shapes
  scale_colour_manual(values=c("gold3", "gold3", "gold3", "darkorange", "darkorange", "darkorange")) +
  scale_shape_manual(values=c(15, 17, 4, 7, 17, 4)) +
...
swr = ggplot(evecSWR, aes(x=PC1, y=PC2)) +
  ...
  scale_shape_manual(values=c(16, 12, 13, 16, 8)) +
  ...
  geom_text_repel(data=subset(evecSWR, time_window=="SWRpre"), aes(label=sample_name), segment.size=.2, segment.alpha=.1, size=2.4, force=20, box.padding=0.5, colour="gray20")
...
```{r}
pdf('WR_v4_PCAs.pdf', height=6, width=8, useDingbats = F)
fig
dev.off()
```

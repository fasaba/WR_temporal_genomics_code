...
Open the data
```{r}
roh = read.table("WR.RoH.minqual30.minlen100kb.ss.txt",  h=F, fill=T, stringsAsFactors=T, fileEncoding="latin1")
dim(roh)
colnames(roh) = c("RG", "sample", "scaffold", "start", "stop", "length", "sites", "quality")
```


Order the RoH by length per sample, and calculate the fraction of the total length that the RoH contribute to
```{r}
...
Add info about the time window of each sample
```{r}
library(dplyr)

roh_order$time_window = recode(roh_order$sample, mwr_TG0453="NWRpre", mwr_TG0454="NWRpre", mwr_TG0455="NWRpre", mwr_TG0458="NWRpre", mwr_TG0460="NWRpre", mwr_TG0462="NWRpre", 
...
```{r}
roh_order$time_window=factor(roh_order$time_window,levels=c("NWRpre","NWRpost","SWRpre", "SWRpost"))
```


Visualize
```{r}
rohlen = ggplot(roh_order, aes(length, rohfrac, group=sample, colour=time_window)) + geom_line() + theme_bw() +
...
```{r}
pdf("WR_v5_FigS8_RoHlen.pdf", useDingbats = F)
rohlen
dev.off()
```

...
```{r}
png('WR_v3_FigS4_v1.png', height=5, width=5, units = 'in', res = 300)
plot_grid(PC, legend, nrow=1, ncol=2, rel_widths = c(2,.4))
dev.off()
```
...
Ep = Ep +  xlab('UMAP1') + ylab('UMAP2') +
  theme(panel.background = element_rect(fill='white', colour='black'), axis.text.x = element_text(colour='black'), 
        ...
        legend.key.size=unit(.5, 'cm'), legend.text=element_text(size=8),
        axis.title=element_text(size=8), axis.text=element_text(size=6))
  
Ep = Ep + geom_text_repel(data=subset(emb, new_ID == "ZA1845.2"), aes(label=new_ID), segment.size=.18, size=4, colour="gray10", ylim=c(NA, -2.5), xlim=c(NA, 20))
...
```{r}
pdf('WR_v3_Fig2A_v3.pdf', height=5, useDingbats = F)
Ep
dev.off()
```

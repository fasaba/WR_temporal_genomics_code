...
```{r}
# prepare metadata
MD <- read.csv("WR_v3_TableS1_v1.csv", h=T)
...
A = ggplot(evec12, aes(x=`1`, y=`2`))+
  ...
  geom_point(aes(colour=time_window, shape=country), alpha=.7, size=2) + 
  ...
  theme(panel.background = element_rect(fill='white', colour='black'), axis.text.x = element_text(colour='black'), axis.text.y = element_text(colour='black'), 
        ...
        legend.title=element_text(size=0), legend.key=element_rect(fill="white", colour="white"), axis.title=element_text(size=8), legend.text=element_text(size=7), legend.key.size=unit(.4, "cm"))


## PC2 vs PC3
B = ggplot(evec23, aes(x=`1`, y=`2`))+
...
Strip off the legend from both plots
```{r}
A = A + guides(colour=F, shape=F)
B = B + guides(colour=F, shape=F)
```

Place both PCA plots on same panel
```{r}
## 
PC = plot_grid(A, B, labels=c('A', 'B'), label_size = 7, ncol = 1, nrow = 2)
PC
```


Save figure with the two plots and the legend
```{r}
pdf('WR_v3_FigS4_v1.pdf', height=5, width=5, useDingbats = F)
plot_grid(PC, legend, nrow=1, ncol=2, rel_widths = c(2,.4))
dev.off()
```
...
## Reduction of dimensionality with UMAP
...
### UMAP output from GLs instead of from PC data (20.4.2020)

The embeddings were calculated in Computerome, the output is visualized here.

Retrieve the output from UMAP
```{r}
# get embedding from umap
embeddings = read.table ("WR.above13Mb_NoSexChr_NonMaskedRegions_nonRelated_tv.emb.txt", h=F)
...
```{r}
Ep = ggplot(emb, aes(E1, E2)) + 
  geom_point(aes(colour=time_window, shape=country), alpha=.8, size=2.5) +
  
  # assign colors and shapes
  scale_colour_manual(values=c("gold3", "darkorange", "royalblue4", "steelblue2")) +
  scale_shape_manual(values=c(7, 16, 10, 17, 18, 15, 9, 8))
  
Ep = Ep +  xlab('UMAP1') + ylab('UMAP2') +
  theme(panel.background = element_rect(fill='white', colour='black'), axis.text.x = element_text(colour='black'), 
        ...
        legend.key.size=unit(.4, 'cm'), legend.text=element_text(size=7),
        axis.title=element_text(size=7), axis.text=element_text(size=4))
  
Ep = Ep + geom_text_repel(data=subset(emb, new_ID == "ZA1845.2"), aes(label=new_ID), segment.size=.18, size=3.5, colour="gray20", ylim=c(NA, -2.5), xlim=c(NA, 20))
...
```{r}
pdf('WR_v3_Fig2A_v2.pdf', height=4, width=8, useDingbats = F)
Ep
dev.off()
```

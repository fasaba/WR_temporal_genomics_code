...
Filter out the samples that should not be included in this analysis
```{r}
roh_order_ss = filter(roh_order, sample!='mwr_TG0450' & sample!='mwr_TG0451' & sample!='mwr_TG0456' & sample!='mwr_TGF01' & sample!='mwr_TGF02' & sample!='mwr_TGF04' & sample!='mwr_TG0452' & sample!='mwr_TG0461')

roh_order_ss$sample <- factor(roh_order_ss$sample)
```
...
rohage = ggplot(roh_order_ss, aes(length, rohfrac, group=sample, colour=time_window)) + geom_line() + theme_bw() +
  ...
  geom_text_repel(label=roh_order_ss$sample)
rohage  

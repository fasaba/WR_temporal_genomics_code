...
gw_het_ss = ggplot(data_ss, aes(time_window, corrected_gw_het)) + 
  ...
  stat_compare_means(paired=F, method = "wilcox.test", comparisons= list(c("NWR1", "NWR2"), c("SWR1", "SWR2")), label.y=c(0.00022, 0.00019), size=2) +
...
exon_het_ss = ggplot(data_ss, aes(time_window, corrected_exon_het, fill=time_window)) + 
  ...
  # compare medians between violins
  stat_compare_means(paired=F, method = "wilcox.test", comparisons= list(c("NWR1", "NWR2"), c("SWR1", "SWR2")), label.y=c(0.00016, 0.00015), size=2) +
...
```{r eval=FALSE, include=FALSE}
pdf('WR_v2_FigS4_v1.pdf', height=5, useDingbats=F)

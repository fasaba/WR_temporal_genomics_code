...
homratio1 = ggplot(snpeff, aes(time_window, hom_ratio)) + theme_bw() +
  ...
  stat_compare_means(paired=F, method = "wilcox.test", comparisons = list(c("SWRpre", "SWRpost")), method.args = list(alternative = "greater"), label.y = c(0.026, 0.027)) +
  
  geom_text_repel(data=subset(snpeff, time_window!="SWRpost"), aes(label=sample), box.padding=.5, segment.size=.15, size=3, force = 20, colour="grey20", xlim=c(0,"SWRpre"), direction="x")
...
homratio3 = ggplot(snpeff, aes(cluster, hom_ratio)) + theme_bw() +
  ...
  stat_compare_means(paired=F, method = "wilcox.test", comparisons = list(c("SWRpre West", "SWRpre East")), method.args = list(alternative = "greater"), label.y = c(0.026, 0.027)) +

...
```{r}
ggplot(data, aes(lab_ID)) + theme(axis.text.x=element_text(angle=80, hjust=1)) + ylab("Het estimate") +
  geom_point(aes(lab_ID, uncorrected_gw_het), colour="red") +
  geom_point(aes(lab_ID, corrected_gw_het), colour="pink") +
  geom_point(aes(lab_ID, gw_het_GL2), colour="blue") +
  geom_point(aes(lab_ID, gw_het_GL2_3x), colour="green")
```
...
#### Experiment 0. DS with het estimates with GL1

Read in data table
```{r}
DSdata0 = read.csv("depthVShet_15xAll-serial_downsampling_het_curve.csv", h=T, fill=T, stringsAsFactors=T, fileEncoding="latin1")
head(DSdata0)
```
...
```{r}
F4 = ggplot(DSdata0, aes(Depth, Het, colour=Sample)) + geom_point() + geom_line() +
  ylim(limits=c(0,4e-4)) + ggtitle("Downsampling test using -GL1 and no depth filtering") +
  xlim(limits=c(0,23))
...
```{r}
pdf("WR_GWhet_DSexp_comparison.pdf", useDingbats = F)
plot_grid(F4, F1, F3, nrow=3, ncol=1)
dev.off
```

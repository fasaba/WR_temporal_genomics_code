---
title: "White rhino genomic erosion"
...
This code compiles all the relevant calculations and data visualizations done on metrics of group and individual genomic diversity for the white rhino dataset. These metrics include:
- Nucleotide diversity based on transversions
- Genome-wide heterozygosity based on transversions
- Exon heterozygosity based on transversions
- Length of runs of homozygosity (RoH) and estimates of inbreeding calculated as the fraction of the genome in RoH

We compared the four groups specified in the main text in the following manner: NWR1 vs NWR2, and SWR1 vs SWR2. The idea is to verify if significant differences exist between pre- and post-bottleneck samples of each subspecies, and to quantify these differences (delta estimators)

The individual measures of genomic diversity (gw-het, exon het, and RoH length and FRoH), were also computed and compared for a subsampled version of the dataset to check if the genomic erosion patterns observed stand after reducing further sampling bias between pre- and post-bottleneck groups (see section 3).
...
```{r}
# plot violins per group with stats for filtered sample set
gplot(Froh_f_filt_ss, aes(time_window, froh, fill=time_window)) +
  geom_violin(alpha=.3, aes(colour=time_window), lwd=.2) +
